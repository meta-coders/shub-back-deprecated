...
dbApi.getSchedule = function(data, cb) {
  this.checkSession(data.sessionId, (err, userId) => {
    ...
    this.connection.query(sql, (err, schedule) => {
      ...
      const time = 'TIME_FORMAT(start, "%H:%i"), TIME_FORMAT(end, "%H:%i")';
      const sql = `SELECT id, ${time} FROM timetable`;
...
dbApi.insertTimetable = function(data, cb) {
  dbApi.clearTimetable((err) => {
    ...
    let sql = 'INSERT INTO timetable VALUES ';
    const values = data.map(arr => `(null, '${arr[1]}', '${arr[2]}')`);
    sql += values.toString();
    this.connection.query(sql, (err) => {
      if (err) cb(err);
      else cb();
